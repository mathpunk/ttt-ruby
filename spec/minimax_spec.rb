...
describe Minimax do
  ...
  context "maximizing" do
    ...
    context "P2 to block" do
      before(:each) do
        @player1 = DeterministicPlayer.new("P1", "X", [1, 2, 7])
        @player2 = DeterministicPlayer.new("P2", "O", [4, 8])
        @winning_move = Move.new(3)
        @game = Game.new(player1: @player1, player2: @player2)
        5.times { |_| @game.run_ply }
      end
      it "includes the blocking move in maxima" do
        maxima = Minimax.maxima(@game, @player1)
        expect(maxima.any? { |move| move.spot == @winning_move.spot }).to be_truthy
      end
      it "includes no other move" do
        maxima = Minimax.maxima(@game, @player1)
        expect(maxima.size).to eq 1
      end
    end
  ...
  context "minimizing" do
    ...
    context "P2 to win" do
      ...
      it "finds a move that will win for P2" do
        minima = Minimax.minima(@game, @player2)
        expect(minima.any? { |move| move.spot == @winning_move.spot }).to be_truthy
      end

      it "ignores non-winning moves" do
        minima = Minimax.minima(@game, @player2)
        expect(minima.size).to eq 1
      end

      it "works the same with `send` syntax" do
        minima = Minimax.send(:minima, @game, @player2)
        expect(minima.size).to eq 1
      end
